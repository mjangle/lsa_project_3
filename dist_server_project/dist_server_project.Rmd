...
```
As we can from our two sample t-test's the means for Test 1 = 403.6, Test 2 = 396.04, and  Test 3 = 378.12 and all three tests fall below a significant level of .05. This low significance level indicates that all three are not part of the same underlying distribution. 
...
ggplot(aes(x = X2)) +
    geom_histogram(binwidth = 1) +
    theme_bw() +
    labs(title = "German Final Transfer Times") +
    xlab("Transfer Time in Milliseconds")
```
...
After we ran our tests on the German server we ran our tests last on the server in Spain. As before we plotted the time series data and also plotted the histograms. 
...
The time series data for the Spain server seems to show quite a bit more variation than either Clipper or the German server. 
...
```
On just ranges the difference is very apparent. The Spanish server results seem to have a wider range of transfer times. 
...
```
These results again seem to differ quite a bit from the German server results. The mean time is 748.32 between all three sets, which is significantly more than the German server which was approximately 392 milliseconds. The max transfer of 2421 milliseconds is the highest transfer time we've encountered for any of our tests.  Our range also shows quite a bit of variance from a min transfer time of 287 to 2421 milliseconds. 
...
To test this we took our cleaned data and ran a t-test. 

```{r,  echo=FALSE}
finalResults <- z.test(spainFinalResults$X2, GermanyFinalResults$X2, var.equal=TRUE) 
finalResults
```
To our surprise we got an extremely low final p-value, indicating we should not reject the null hypothesis. The German data and the Spanish data were part of the same underlying distribution. 

To further examine this we took the resulting data and coombined them into a dataframe to plot into a histogram. 

```{r, echo=FALSE, message=FALSE}
finalHistResults <- merge(GermanyFinalResults, spainFinalResults, by )

FinalHistResults %>% 
ggplot(aes(x = X2)) +
    geom_histogram(binwidth = 15) +
    theme_bw() +
    labs(title = "Spain Final Transfer Times") +
    xlab("Transfer Time in Milliseconds")
```
